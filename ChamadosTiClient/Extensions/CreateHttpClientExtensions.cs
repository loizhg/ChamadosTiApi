...
namespace ChamadosTiClient.Extensions
{
    ...
    {
        public static HttpResponseMessage CreateAsJsonAsync<T>(this HttpClient httpClient, string requestUri, T data)
            => httpClient.SendAsync(new HttpRequestMessage(HttpMethod.Post, requestUri) { Content = Serialize(data) }).Result;
